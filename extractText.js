...
export async function exctractSaleEndDate(text) {
  ...
  if (!text || typeof text !== 'string' || !text.trim()) {
    logger.error('[formatDataToJson] Invalid input text.');
    return null;
  }
  ...
  const prompt =
    ...
    '3. If the year is missing, use the current year: ' +
  ...
  try {
    ...
    try {
      ...
      logger.error('[formatDataToJson] Failed to parse Vertex AI response as JSON:', err);
      return null;
    }
    ...
    if (
      ...
      logger.warn('[formatDataToJson] Vertex AI response missing or invalid endDate.');
      return null;
    }
  } catch (error) {
    logger.error('[formatDataToJson] Vertex AI API error:', error);
    return null;
  }
}
