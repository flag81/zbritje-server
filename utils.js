  ...
  pages.forEach((page) => {
    ...
    if (wordsWithBoxes.length > 0) {
      ...
      for (let i = 1; i < wordsWithBoxes.length; i++) {
        ...
        if (verticalDistance < VERTICAL_THRESHOLD && horizontalDistance < HORIZONTAL_THRESHOLD * 3) {
          // Adjusted horizontal check
    ...
    if (lines.length > 0) {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const lastLineBottom = lastLineInBlock.reduce(
          (maxY, word) => Math.max(maxY, word.boundingBox.vertices[2].y),
          -1,
        );
        ...
        const isPriceOrDescription =
          /[0-9€€.]/.test(lineText) || lineText.toLowerCase().includes('kg') || lineText.toLowerCase().includes('l'); // Basic heuristic

        if (verticalDistance < VERTICAL_THRESHOLD * 2 || isPriceOrDescription) {
          // Group lines that are vertically close or contain price/description clues
